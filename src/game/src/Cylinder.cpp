...
Cylinder::Cylinder(int axial_slice_number, int axial_slice_step, int initial_radius, int circular_slice_number)
  ...
  for(int i = 0; i < circular_slice_number; ++i)
    circular_slice_direction_vectors.at(i) = {std::cos(angle * i), std::sin(angle * i), 0};
}

void Cylinder::draw() {
  for(auto i = 0; i < radiuses.size(); ++i) {
    // TODO FIX 切断がうまくいかない
  ...
  for(auto point : circular_slice_direction_vectors) {
    ...
    for(int i = 0; i < radiuses.size(); ++i)
      glVertex3d(point.x * radiuses.at(i), point.y * radiuses.at(i), axial_slice_step * i);
    glVertex3d(0, 0, Space::cylinderical_to_world({axial_slice_number, 0}).z);
    glEnd();
  }
}

void Cylinder::cut_point(Space::Cylinderical point) {
  if(point.z < 0 || axial_slice_number <= point.z) return;
  if(radiuses.at(point.z) > point.r) radiuses.at(point.z) = point.r;
}

void Cylinder::add_point(Space::Cylinderical point) {
  if(point.z < 0 || axial_slice_number <= point.z) return;
  if(radiuses.at(point.z) < point.r) radiuses.at(point.z) = point.r;
}

void Cylinder::cut_path(Space::Cylinderical point1, Space::Cylinderical point2) {
  ...
  Space::Cylinderical start = (point1.z < point2.z ? point1 : point2);
  Space::Cylinderical end = (point1.z < point2.z ? point2 : point1);
  for(int z = start.z; z <= end.z; ++z) {
    cut_point({z, start.r + dydx * (z - start.z)});
  }
}

void Cylinder::add_path(Space::Cylinderical point1, Space::Cylinderical point2) {
  if(point1.z == point2.z) {
    add_point(point1);
    add_point(point2);
    return;
